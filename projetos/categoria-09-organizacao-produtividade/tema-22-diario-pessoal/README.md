# Tema 22 – Diário pessoal

Registros pessoais, anotações e lembretes.

Categoria: **[Organização e Produtividade](../README.md)** · Fichas: [fase1](../../fase1-bimestre1.md) · [fase2](../../fase2-bimestre2.md) · [fase3](../../fase3-bimestre3.md) · [fase4](../../fase4-bimestre4.md)

## O que o aplicativo faz

- Criar novas anotações com título e texto.
- Anexar uma foto da galeria à anotação.
- Editar e excluir anotações existentes.
- Listar as anotações por data (mais recentes primeiro).

## Telas sugeridas

| Tela | Conteúdo |
|------|----------|
| Lista | Anotações com título e data |
| Detalhe | Título, texto, foto e data + botão Voltar |
| Cadastro / Edição | Formulário com campos título, texto, foto e data |

## Banco de dados (SQLite)

`notas`:

| Campo | Tipo | Descrição |
|-------|------|-----------|
| id | INTEGER | Identificador |
| titulo | TEXT | Título da anotação |
| texto | TEXT | Texto da anotação |
| caminho_foto | TEXT | Caminho da foto anexada (opcional) |
| data | TEXT | Data da anotação |

## Escopo das 4 entregas

| Entrega | Data | Foco |
|---------|------|------|
| 1 | 24/08/2026 | Proposta + 2 telas (lista + detalhe) com dados estáticos |
| 2 | 14/09/2026 | Formulário + AsyncStorage |
| 3 | 19/10/2026 | SQLite + CRUD |
| 4 | 16/11/2026 | App completo + UX revisada; apresentação |

> Os requisitos e critérios de cada entrega estão nas fichas compartilhadas do Trabalho em Grupo (`fase1-bimestre1.md` a `fase4-bimestre4.md`).
